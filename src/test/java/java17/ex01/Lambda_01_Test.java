...
//        List<Person> result = filter(personList, null);
		...
		List<Person> result = personList.stream().map(p -> p).filter(person -> person.getAge() >= 18).toList();
...
//        List<Person> result = filter(personList, null);
        List<Person> result = personList.stream().filter(person -> "first_10".equals(person.getFirstname())).toList(); 
...
//        List<Person> result = filter(personList, null);
        ...
        List<Person> result = personList.stream()
        		.filter(person -> person.getAge() > 49)
                .filter(person -> DigestUtils.sha512Hex(person.getPassword()).equals(passwordSha512Hex))
                .toList();
